class PlantSearch extends HTMLElement {
	...
	connectedCallback() {
		...
		camera.addEventListener('photo', async (e) => {
			const urlKey = 'https://vision.googleapis.com/v1/images:annotate?key=AIzaSyDbccs3dhKonS0crABRuHJR-n-un99oqEM';
			const blob = e.detail;
			const dataUrl = await camera.toDataURL(blob);
			
			const visionAPI = await fetch(urlKey, {
				method: "POST",
				body: JSON.stringify({
					"requests":[
					  {
			...
			if( visionAPI){
				//search-results.json() = result.json.then((data));
